...
namespace SAP.AppsForOffice.Workflow.Pages.GatewayPages
{
    public partial class DefaultForGateway : System.Web.UI.Page
    {
        ...
        {
            return ConfigurationManager.AppSettings["ErrorMsgWindowUrl"];
        }

        /// <summary>
        /// checks if the task type is leave. If it is, then it will return true
        /// </summary>
        /// <param name="taskId"></param>
        /// <returns></returns>
        [WebMethod]
        [ScriptMethod(UseHttpGet = true)]
        public static bool IsTaskTypeLeave(string taskReqType)
        {
            if (taskReqType == ConfigurationManager.AppSettings["GatewayRequestType"])
            {
                return true;
            }
            return false;
